...
function getForecast(force) {
  if (!localStorage.getItem("weatherData") || force) {
    Logger.log(`Getting Forecast..`, 100)
    ...
    .then(function(data) {
      ...
      Logger.log('Got weather data from internet.')
    })
    ...
    Logger.log('Got weather data from localstorage.')
  }
}
...
function checkTime() {
  ...
  if (previousHour !== h) {
    Logger.log("ITS A NEW HOUR: " + h)
    previousHour = h
    if ((present > 1) && !quiet) tts.speak(moment(today).format('h:mma'), {alert: true, bgm: false, volume: 0})
  }
    ...
    switch (present) {
      ...
      case 0:
        ...
        if (!quiet) {
          ...
          tts.speak(text, {alert: true, bgm: true, volume: 7})
        } 
        break;
    }
  } else {
